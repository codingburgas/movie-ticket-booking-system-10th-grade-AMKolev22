...
std::future<NoReturn> Eccc::Core::Database::connectToDb() {
	try {
		sql.open(soci::postgresql, "dbname=movie-db user=movieAdmin password=123456_school! host=movie-db.postgres.database.azure.com port=5432");
		return {};
	}
...
Eccc::Core::Database::ASYNC_NoReturn Eccc::Core::Database::async_connectToDb() {
	try {
		...
		sql.open(soci::postgresql, "dbname=postgres user=movieAdmin password=123456_school! host=movie-db.postgres.database.azure.com port=5432");
		return std::async(std::launch::async, []() -> NoReturn {
			return {};
		});
	}
